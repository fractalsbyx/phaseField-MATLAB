...
function [dArrdx, dArrdy] = gradArr_2D(Arr, rho, dx, dy)
  drhodx_central  = (circshift(rho, [0, -1]) - circshift(rho, [0, 1])) ./ 2.0 ./ dx;
  drhody_central  = (circshift(rho, [-1, 0]) - circshift(rho, [1, 0])) ./ 2.0 ./ dy;
  ...
  dArrdx = zeros(size(drhodx_central), class(Arr));
  dArrdy = zeros(size(drhody_central), class(Arr));
